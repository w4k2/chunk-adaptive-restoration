...
def run():
    models = [AUE(GaussianNB()), MLPClassifier(), AWE(GaussianNB()), OnlineBagging(GaussianNB()), SEA(GaussianNB())]
    ...
    for clf in models:
        for seed in seeds:
            for variable_chunk_size in use_variable_chunk_size:
                experiment(clf, random_state=seed, variable_chunk_size=variable_chunk_size)
        break
